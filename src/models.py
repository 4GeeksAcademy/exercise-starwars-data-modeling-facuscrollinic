...
class People(Base):
    ...
    planets_relation = relationship('Planets', back_populates='residents')
    vehicles_relation = relationship('Vehicles', back_populates='pilots')
...
class Vehicles(Base):
    ...
    pilots = Column(Integer, ForeignKey('pilots.id'), nullable=False)
    films = Column(Integer, ForeignKey('films.id'), nullable=False)
    url = Column(String(250), nullable= False)
